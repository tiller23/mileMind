...
class TestEvaluateFatigueStateInputValid:
    ...
    def test_custom_tau_values(self, steady_loads_30: list[float]) -> None:
        ...
        m = EvaluateFatigueStateInput(
            daily_loads=steady_loads_30, fitness_tau=56, fatigue_tau=10
        )
...
class TestEvaluateFatigueStateOutput:
    ...
    def test_without_series(self) -> None:
        ...
        out = EvaluateFatigueStateOutput(
            ctl=30.0, atl=45.0, tsb=-15.0, recovery_status="fatigued"
        )
    ...
    def test_model_dump_keys(self) -> None:
        ...
        out = EvaluateFatigueStateOutput(
            ctl=1.0, atl=2.0, tsb=-1.0, recovery_status="neutral"
        )
        d = out.model_dump()
        assert set(d.keys()) == {
            "ctl", "atl", "tsb", "recovery_status",
...
class TestEvaluateFatigueStateHandlerCorrectness:
    ...
    def test_ctl_matches_banister(self, steady_loads_30: list[float]) -> None:
        ...
        result = evaluate_fatigue_state_handler(
            {"daily_loads": steady_loads_30, "fitness_tau": 42, "fatigue_tau": 7,
    ...
    def test_mathematically_exact_ctl(self, steady_loads_30: list[float]) -> None:
        ...
        result = evaluate_fatigue_state_handler(
            {"daily_loads": steady_loads_30, "fitness_tau": tau, "fatigue_tau": 7,
             "include_series": False}
        )
    ...
    def test_custom_tau_values(self, steady_loads_30: list[float]) -> None:
        ...
        result = evaluate_fatigue_state_handler(
            {"daily_loads": steady_loads_30, "fitness_tau": 28, "fatigue_tau": 5,
             "include_series": False}
        )
...
class TestEvaluateFatigueStateHandlerSeries:
    ...
    def test_series_last_value_matches_scalar(self, mixed_loads: list[float]) -> None:
        ...
        result = evaluate_fatigue_state_handler(
            {"daily_loads": mixed_loads, "fitness_tau": 42, "fatigue_tau": 7,
             "include_series": True}
        )
    ...
    def test_series_matches_banister_compute_tsb_series(
        self, mixed_loads: list[float]
    ) -> None:
    ...
    def test_single_day_with_series(self, minimal_loads: list[float]) -> None:
        ...
        result = evaluate_fatigue_state_handler(
            {"daily_loads": minimal_loads, "fitness_tau": 42, "fatigue_tau": 7,
             "include_series": True}
        )
...
class TestEvaluateFatigueStateHandlerErrors:
    ...
    def test_fatigue_tau_equal_to_fitness_tau_raises(
        self, minimal_loads: list[float]
    ) -> None:
        """fatigue_tau == fitness_tau must raise ValueError."""
        with pytest.raises(ValueError, match="fatigue_tau"):
            evaluate_fatigue_state_handler(
                {"daily_loads": minimal_loads, "fitness_tau": 10, "fatigue_tau": 10,
                 "include_series": False}
            )

    def test_fatigue_tau_greater_than_fitness_tau_raises(
        self, minimal_loads: list[float]
    ) -> None:
        """fatigue_tau > fitness_tau must raise ValueError."""
        with pytest.raises(ValueError, match="fatigue_tau"):
    ...
    def test_error_message_mentions_both_tau_values(
        self, minimal_loads: list[float]
    ) -> None:
        """Error message must mention both tau values for debuggability."""
        with pytest.raises(ValueError) as exc_info:
            evaluate_fatigue_state_handler(
                {"daily_loads": minimal_loads, "fitness_tau": 14, "fatigue_tau": 20,
                 "include_series": False}
            )
...
class TestEvaluateFatigueStateHandlerOutputSchema:
    ...
    def test_output_keys_without_series(self, steady_loads_30: list[float]) -> None:
        ...
        expected_keys = {"ctl", "atl", "tsb", "recovery_status",
                         "ctl_series", "atl_series", "tsb_series"}
...
class TestRegistration:
    ...
    def test_get_anthropic_tools_returns_list(
    ...
    def test_anthropic_tool_has_required_keys(
        self, registry_with_tool: ToolRegistry
    ) -> None:
    ...
    def test_input_schema_has_daily_loads_property(
        self, registry_with_tool: ToolRegistry
    ) -> None:
    ...
    def test_input_schema_has_fitness_tau_property(
        self, registry_with_tool: ToolRegistry
    ) -> None:
    ...
    def test_input_schema_has_fatigue_tau_property(
        self, registry_with_tool: ToolRegistry
    ) -> None:
...
class TestRegistryExecuteRoundTrip:
    ...
    def test_execute_invalid_empty_loads(
        self, registry_with_tool: ToolRegistry
    ) -> None:
    ...
    def test_execute_invalid_negative_load(
        self, registry_with_tool: ToolRegistry
    ) -> None:
